...
class Config:
    ...
    def __init__(self, args: Namespace = None):
        ...
        self.min_tokens = getattr(args, "min_tokens", 1)
        ...
        if self.few_shot_k is None:
            self.few_shot_k = 1000
        ...
        self.tool_model = getattr(args, "tool_model", self.model)
        ...
        if self.agent_type in ["cot", "direct"] and self.self_consistency_k > 1:
            if self.temperature <= 0:
                logger.warning(
                    "Cannot have multiple self-consistency attempts with greedy strategy! Switching decoding strategy to sampling..."
                )
                self.temperature = 0.7
        elif self.agent_type not in ["cot", "direct"]:
            if self.self_consistency_k != 1:
                logger.warning(
                    "Cannot use self-consistency with other agent types than CoT! Disabling self-consistency..."
                )
        ...
        self.prompt_file = getattr(
            args, "prompt_file", os.path.join(PROMPT_DIR, self.dataset + ".py")
        )
        self.agent_file = getattr(
            args, "agent_file", os.path.join(SPEC_DIR, self.agent_type + ".txt")
        )
