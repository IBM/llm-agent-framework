...
# Local
from lm_agent.agent import LLMAgent
from lm_agent.constants import *
from lm_agent.eval_utils import get_init_metrics
from lm_agent.utils import *
...
def _take_fallback(alt_ans: List[str], fallback_ans: List[str]):
    ...
    for ans in max_lengths:
        matching_fallbacks = [
            f_ans
            for f_ans in fallback_ans
            if _canon_answer(f_ans[0]) == _canon_answer(ans[0])
        ]
        for match in matching_fallbacks:
            ans = ans + match
        plus_fallback.append(ans)

    max_lengths = _get_max(plus_fallback)
    if any([len(ans) > 1 for ans in max_lengths]):
        return max_lengths[0][0]
...
def _eval(dataset_args, config, prior_predictions):

    (input_text, answer, idx) = dataset_args
...
def main(args: Namespace):
    ...
    metrics = (
        get_init_metrics()
        if any([args.dataset.startswith(opt) for opt in ["hotpot", "trivia"]])
        else {"correct": 0}
    )

    prior_predictions = extract_predictions(config.predictions_file)
    fallback_predictions = extract_predictions(config.fallback_predictions_file)
    ...
    with (Pool(args.process_ct) if parallel_exec else nullcontext()) as p, tqdm(
        ...
        for dataset_chunk in chunk(dataset, args.chunk_size):
            ...
            for json_data in iterator:
                idx, input_text, answer, responses = (
                    json_data["id"],
                    json_data["input_text"],
                    json_data["answer"],
                    json_data["generated_responses"],
                )
                check_responses = responses
                if idx in fallback_predictions:
                    fallback_responses = fallback_predictions[idx][
                        "generated_responses"
                    ]
                ...
                is_correct, extracted_answer = check_correctness(
                    gen_ans, prediction, answer, metrics, config
                )
...
if __name__ == "__main__":
    ...
    """
    How to call:
        python -m evaluate --model <model-type> --dataset <dataset-name>

    Example:
        python -m evaluate --model meta-llama/llama-2-70b --agent_type pass --dataset hotpot_qa --dataset_size -1 --split validation --debug --restart
        python -m evaluate --model meta-llama/llama-2-70b --agent_type react --dataset hotpot_qa --dataset_size -1 --split validation --debug --restart
        python -m evaluate --model meta-llama/llama-2-70b --agent_type rewoo --dataset hotpot_qa --dataset_size -1 --split validation --debug --restart
        python -m evaluate --model meta-llama/llama-2-70b --agent_type cot --self_consistency_k 5 --dataset hotpot_qa --dataset_size -1 --split validation --debug --restart
    ...
    parser.add_argument(
        ...
        help=" Name of dataset you are training",
    )
    ...
    parser.add_argument(
        ...
        default=None,
        type=str,
        help="Fallback agent to use if no answer is provided by specified agent",
    )
    ...
    parser.add_argument(
        ...
        default=1,
        type=int,
        help="K retries for self-consistency",
    )
