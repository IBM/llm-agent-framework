...
class Prompt:
    ...
    INSTRUCTIONS_KW, EXAMPLES_KW, INPUT_KW, STOP_KW = (
        "instructions",
        "examples",
        "input",
        "stop_text",
    ...
    def __init__(self, config: Config):
        ...
        prompt_contents = _load_prompt_contents(
            config.prompt_file, agent_type=config.agent_type
        )
        ...
        self._examples = [
            _clean_text_content(ex) for ex in self._examples[: self.config.few_shot_k]
        ]
        ...
        var_marker = "{{"
        assert (
            var_marker in self._initial
        ), f"Must specify a variable with {var_marker} in prompt!"
    ...
    def fill_prompt(self, **kw_args):
        ...
        for k, v in kw_args.items():
            instructions, examples, initial = (
                sub_in_kv(instructions, k, v),
                [sub_in_kv(ex, k, v) for ex in examples],
                sub_in_kv(initial, k, v),
            )
        self.instructions, self.examples, self.initial = (
            str(instructions),
            [str(ex) for ex in examples],
            str(initial),
        )
...
def _clean_text_content(text: str):
    ...
    return "\n".join(
        line.strip().replace(tab_sep, "\t") for line in text.split("\n")
    ).strip()
